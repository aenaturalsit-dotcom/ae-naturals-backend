...
export class PaymentsController {
  ...
  async payuVerify(@Body() body: any, @Res() res: Response) {
    ...
    try {
      ...
      if (status !== 'success') {
        return res.redirect(
          `${fallbackFrontendUrl}/checkout?error=payment_failed`,
        );
      }
      ...
      if (result.success) {
        return res.redirect(
          `${result.frontendUrl}/order-success/${result.orderId}`,
        );
      }
    } catch (error) {
      console.error('PayU Verification Error:', error);
      return res.redirect(
        `${fallbackFrontendUrl}/checkout?error=hash_mismatch`,
      );
    }
  }

  // ✅ NEW: Polling endpoint for frontend verify check
  @UseGuards(AuthGuard('jwt'))
  @Get('verify')
  async verifyPayment(@Query('orderId') orderId: string, @Req() req: any) {
    const userId = req.user?.id || req.user?.userId;
    return this.paymentsService.verifyPaymentStatusForFrontend(orderId, userId);
  }
}
